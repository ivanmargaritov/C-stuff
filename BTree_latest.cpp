...
private:
	...
	int longestConsecutive(Node<T>* subTreeRoot, int currentLen, int expected);
	void BTree<T>::simplePrintStack(Node<T>* subTreeRoot) const;
...
int BTree<T>::sumOfLeftLeaves(Node<T>* subTreeRoot, bool isLeft)
{
	...
	{
		return sumOfLeftLeaves(subTreeRoot->left, 1) + sumOfLeftLeaves(subTreeRoot->right, 0);
	}
}
...
template <class T>
int BTree<T>::longestConsecutive(Node<T>* subTreeRoot, int currentNum, int expected)
{
	if (subTreeRoot == nullptr)
	{
		return currentNum;
	...
	int left = longestConsecutive(subTreeRoot->left, currentNum, expected + 1);
	int right = longestConsecutive(subTreeRoot->right, currentNum, expected + 1);
...
void BTree<T>::simplePrintStack(Node<T>* subTreeRoot) const
{
	stack <waitingStep<T>> operations;

	operations.push(waitingStep<T>(STEP_TRAVERSE_SUBTREE, subTreeRoot));

	cout << "{";

	while (!operations.empty())
	{
		waitingStep<T> topOperation = operations.top();
		Node<T>* topNode = topOperation.second;
		operations.pop();
		...
		{
			operations.push(waitingStep<T>(STEP_TRAVERSE_SUBTREE, topNode->right));
			operations.push(waitingStep<T>(STEP_EXTRACT_ROOT, topNode));
			operations.push(waitingStep<T>(STEP_TRAVERSE_SUBTREE, topNode->left));
		}
	}
	cout << "}\n";
}
...
template <class T>
void BTree<T>::simplePrintStack() const
{
	simplePrint(root);
}
...
bool BTree<T>::isBOT(Node<T>* subTreeRoot, int min, int max)
{
	...
	return isBOT(subTreeRoot->left, min, subTreeRoot->data - 1) && isBOT(subTreeRoot->right, subTreeRoot->data + 1, max);
}
...
int main()
{
	...
	BTree<int> inserted=tree.insertedBOT(13);
	...
	cout << endl << endl << "-----POST ORDER PRINT-----" << endl;
	calc.simplePrint();
	cout << endl;
	tree.simplePrint();
	cout << endl;

	cout << endl << "-----SUM OF LEFT LEAVES-----" << endl;
	cout << tree.sumOfLeftLeaves() << endl;

	cout << endl << "-----LONGEST CONSECUTIVE-----" << endl;
	cout << tree.longestConsecutive() << endl;

	cout << endl << "-----ITERATOR THINGS-----" << endl;
	BTree<int> treeIt;
	treeIt.insertBOT(59)
		.insertBOT(23)
		.insertBOT(68)
	...
	treeIt.prettyPrint();
	cout << endl;
	BTree<int>::LeftRootRightIterator it = treeIt.begin();
	...
	cout << endl << "-----POST ORDER PRINT WITH STACK-----" << endl;
	tree.simplePrintStack();
	cout << endl;

	cout << endl << "-----ARE THE TREES BOT-----" << endl;
	cout << tree.isBOT() << endl << treeIt.isBOT() << endl << t.isBOT() << endl;
